  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex', width: '100%' }}>
      <select
        ...
        style={{
          appearance: 'none', WebkitAppearance: 'none', width: '100%', height: H[size] || H.md,
          background: 'var(--input)', color: 'var(--foreground)', fontFamily: 'var(--font-sans)',
          fontSize: 'var(--text-ui)', border: '1px solid ' + (focus ? 'var(--ring)' : 'var(--border)'),
      ...
      <span style={{ position: 'absolute', right: 9, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: 'var(--muted-foreground)', fontSize: 10 }}>▾</span>
    </div>
  );
}
