...
export class ChatSession {
  ...
  private async restart(): Promise<void> {
    ...
    if (!options) return;
    // Carried across the restart. `stop()` empties the line, which is right
    // when a session is being torn down and wrong here: this is one turn in a
    // conversation that continues, and anything typed behind the `/clear` was
    // typed *for* the fresh process about to start (#89). Discarding it would
    // be the silent loss this whole path exists to stop.
