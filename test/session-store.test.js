...
describe('SessionStore', function() {
  ...
  describe('global project-session metadata', function () {
    it('propagates authoritative load and active-reset failures', async function () {
      sessionStore.database.close();
      sessionStore = new SessionStore({
        database: failingSessionDatabase(tempDir, 'forced scoped-global database failure'),
        scopedGlobalStore: true,
      });

      await assert.rejects(
        () => sessionStore.loadSessions(),
        /forced scoped-global database failure/,
      );
      await assert.rejects(
        () => sessionStore.resetActiveFlags(),
        /forced scoped-global database failure/,
      );
    });

    it('does not prune scoped rows until a complete authoritative load succeeds', async function () {
      sessionStore.database.close();
      sessionStore = new SessionStore({ dataDir: tempDir, scopedGlobalStore: true });
      const workspaceRoot = await fs.mkdtemp(path.join(tempDir, 'project-prune-gate-'));
      const scope = {
        workspaceRoot,
        ownerKey: '3'.repeat(64),
      };
      const first = createSessionRecord({
        id: 'global-prune-first',
        ownerUserId,
        workingDir: workspaceRoot,
        storageScope: scope,
      });
      const second = createSessionRecord({
        id: 'global-prune-second',
        ownerUserId,
        workingDir: workspaceRoot,
        storageScope: scope,
      });
      ...
      assert.strictEqual(
        await sessionStore.saveSessions(new Map([[first.id, first]])),
        true,
      );
      assert.deepStrictEqual(
        sessionStore.database.raw.prepare(
          'SELECT id FROM runtime_sessions ORDER BY id',
        ).all().map((row) => row.id),
        [first.id, second.id],
        'a pre-load snapshot must not infer that an unseen scoped row was deleted',
      );

      assert.strictEqual((await sessionStore.loadSessions()).size, 2);
      assert.strictEqual(
        await sessionStore.saveSessions(new Map([[first.id, first]])),
        true,
      );
      assert.deepStrictEqual(
        ...
        [first.id],
        'a complete load authorises later missing-row pruning',
      );
    });

    it('round-trips immutable scope, draft, tab, and active state through app.sqlite', async function () {
      sessionStore.database.close();
      sessionStore = new SessionStore({ dataDir: tempDir, scopedGlobalStore: true });
      const workspaceRoot = await fs.mkdtemp(path.join(tempDir, 'project-scope-'));
      const scope = {
        workspaceRoot,
        ownerKey: '4'.repeat(64),
      };
      const draft = {
        text: 'finish this after restart',
        attachments: [{
          url: '/api/sessions/global-scoped/chat-attachments/0123456789ab-photo.png',
          name: 'photo.png',
          mime: 'image/png',
          size: 123,
        }],
      ...
      assert.strictEqual(await sessionStore.saveSessions(new Map([[record.id, record]])), true);
      await sessionStore.setActive(record.id, true, scope);

      const row = sessionStore.database.raw.prepare(
        'SELECT storage_workspace_root, storage_owner_key, chat_draft_json, '
        + 'tab_open, tab_order, active FROM runtime_sessions WHERE id = ?',
      ).get(record.id);
      assert.deepStrictEqual(
        {
          storage_workspace_root: row.storage_workspace_root,
          storage_owner_key: row.storage_owner_key,
          chat_draft: JSON.parse(row.chat_draft_json),
          tab_open: row.tab_open,
          tab_order: row.tab_order,
          active: row.active,
        },
        {
          storage_workspace_root: workspaceRoot,
          storage_owner_key: scope.ownerKey,
          chat_draft: draft,
          tab_open: 0,
          tab_order: 5,
          active: 1,
      ...
      const workspaceDatabase = path.join(workspaceRoot, '.cc-web', 'session-state.sqlite');
      assert.strictEqual(
        await fs.access(workspaceDatabase).then(() => true, () => false),
        false,
        'global metadata writes must not create a workspace SQLite database',
      );

      sessionStore.database.close();
      sessionStore = new SessionStore({ dataDir: tempDir, scopedGlobalStore: true });
      const restored = (await sessionStore.loadSessions({ preserveActive: true })).get(record.id);
      assert.deepStrictEqual(restored.storageScope, scope);
      assert.deepStrictEqual(restored.chatDraft, draft);
      assert.strictEqual(restored.tabOpen, false);
      assert.strictEqual(restored.tabOrder, 5);
      assert.strictEqual(restored.active, true);
    });

    it('keeps terminal output history out of global SQLite and creates no project database', async function () {
      sessionStore.database.close();
      sessionStore = new SessionStore({ dataDir: tempDir, scopedGlobalStore: true });
      const workspaceRoot = await fs.mkdtemp(path.join(tempDir, 'project-output-'));
      const scope = {
        workspaceRoot,
        ownerKey: '5'.repeat(64),
      };
      const record = createSessionRecord({
        id: 'global-output',
        ownerUserId,
        ...
        outputBuffer: [
          ...
          '\u001b[31msecond project-local line\u001b[0m',
        ],
      });

      assert.strictEqual(await sessionStore.saveSessions(new Map([[record.id, record]])), true);
      const row = sessionStore.database.raw.prepare(
        'SELECT output_buffer_json FROM runtime_sessions WHERE id = ?',
      ).get(record.id);
      assert.strictEqual(
        row.output_buffer_json,
        '[]',
        'app.sqlite stores coordination metadata, not duplicate terminal history',
      );

      sessionStore.database.close();
      sessionStore = new SessionStore({ dataDir: tempDir, scopedGlobalStore: true });
      const restored = (await sessionStore.loadSessions()).get(record.id);
      assert.deepStrictEqual(restored.storageScope, scope);
      assert.deepStrictEqual(restored.outputBuffer, []);

      const workspaceDatabase = path.join(workspaceRoot, '.cc-web', 'session-state.sqlite');
      assert.strictEqual(
        await fs.access(workspaceDatabase).then(() => true, () => false),
        false,
        'saving and restoring global metadata must never materialise project SQLite',
      );
    });
  });
});
...
function failingSessionDatabase(storageDir, message) {
  return {
    storageDir,
    dbPath: path.join(storageDir, 'forced-failure.sqlite'),
    raw: {
      prepare() {
