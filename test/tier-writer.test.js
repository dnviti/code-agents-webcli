...
describe('tier writer', function () {
  ...
  describe('pi', function () {
    ...
    it('never overwrites an agent file the user wrote', function () {
      const dir = path.join(ctx.workingDir, '.pi', 'agents');
      fs.mkdirSync(dir, { recursive: true });
      ...
      assert.strictEqual(fs.readFileSync(mine, 'utf8'), 'hand written, do not touch\n');
      assert.deepStrictEqual(result.written, []);
      assert.strictEqual(result.skipped.length, 1);
      assert.strictEqual(result.skipped[0].file, mine);
    });

    it('says what skipping cost, not just that it happened', function () {
      // A user reading "not generated by this app" has no way to tell that the
      // tier they just configured is doing nothing. The reason has to carry the
      // consequence, because the UI shows the tier fields filled in either way.
      const dir = path.join(ctx.workingDir, '.pi', 'agents');
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(path.join(dir, 'mid.md'), 'hand written\n');

      const result = applyTiers(profile({ tiers: { mid: 'x' } }), ctx);
      assert.match(result.skipped[0].reason, /not applied/);
    });

    it('reports every tier it skipped, not just the first', function () {
      // All four skipped is the realistic case for anyone who already keeps a
      // hand-written ladder — and it is the case where the profile silently
      // does nothing at all, so none of it may be swallowed.
      const dir = path.join(ctx.workingDir, '.pi', 'agents');
      fs.mkdirSync(dir, { recursive: true });
      ...
      assert.deepStrictEqual(
        result.skipped.map((s) => path.basename(s.file)).sort(),
        ['floor.md', 'high.md', 'mid.md', 'top.md'],
      );
    });

    it('does overwrite a file it generated earlier', function () {
