...
describe('project routes', function () {
  ...
  it('replays the build log then live events over SSE', async function () {
    ...
    manager.events.emit('build', {
      projectId: 'project-1',
      event: { t: 'state', state: 'stopped', at: new Date().toISOString() },
    });

    const text = await readPromise;
    const lines = text.split('\n');
    const dataLines = lines.filter((line) => line.startsWith('data:'));
    assert.strictEqual(dataLines.length, 3, `expected 3 data frames, got: ${text}`);
    ...
    assert.ok(dataLines.some((line) => line.includes('"state":"stopped"')), 'terminal stopped event delivered');
  });
  ...
  it('cleans up when the SSE request is aborted', async function () {
    ...
    const listenerCount = manager.events.listenerCount('build');
    assert.strictEqual(listenerCount, 1, 'one build listener should be attached');
